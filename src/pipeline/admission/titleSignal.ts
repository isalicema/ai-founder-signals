...
export function evaluateTitle(title: string): TitleSignal {
  ...
  if (byKind.weak_positive.length > 0 || GUEST_SEPARATOR.test(title)) {
    ...
    return { verdict: 'weak_positive', score: 0.5, matched };
  }
  return { verdict: 'none', score: 0.3, matched };
}
