...
- “👍 喜欢 / 👎 不喜欢”只记录明确个人偏好，不改变算法高亮、质量分、已读状态或页面位置。
- 低分内容可恢复为普通信号并记录为 `restored`；恢复后是否高亮仍由固定质量线决定。

### 升级说明

- 数据库模式需应用 `supabase/migrations/20260909170649_add_dislike_feedback_signal.sql`，以允许 `like`、`dislike` 和 `restored` 反馈值。
